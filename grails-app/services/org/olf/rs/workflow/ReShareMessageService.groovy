...
class ReShareMessageService implements ApplicationListener {
		...
		if (!patronRequest.awaitingProtocolResponse) {
			...
			if (patronRequest.pendingAction) {
				...
				if (tenantId) {
					...
					queue(tenantId, patronRequest.pendingAction.id, patronRequest.id, patronRequest.version);
				}
			}
		}
	}
	...
	/**
	 *  Gets hold of the specified request and checks we have the right version
	 *
	 * @param requestId the id of the patrong request we are interested in
	 * @param version is the version we are interested in
	 * 
	 * @return The patron request if we have found one
	 */
	...
	public void processAnIncomingMessage(Map messageDetails) {
		...
		Long version = messageDetails[VERSION];
	 ...
	 */
	public void queue(String tenantId, String actionCode, String requestId, Long version) {
		log.debug("queue(tenant:${tenantId}, action:${actionCode}, requestId:${requestId}, requestId:${version})");
