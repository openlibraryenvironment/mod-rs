...
/**
 * Processes incoming messages from the requester
 * @author Chas
 *
 */
...
//                            reshareApplicationEventHandlerService.auditEntry(pr, pr.state, pr.state, 'Incomnig message: Received, State change: ' + pr.state.code + ' -> '  + pr.state.code, null);
                        ...
                        case 'ShippedReturn':
                            Status newState = reshareApplicationEventHandlerService.lookupStatus(StateModel.MODEL_RESPONDER, Status.RESPONDER_ITEM_RETURNED);
                            pr.volumes?.each { vol ->
                            ...
                            // Adding an audit entry so we can see what states we are going to for the event
                            // Do not commit this uncommented, here to aid seeing what transition changes we allow
//                            reshareApplicationEventHandlerService.auditEntry(pr, pr.state, pr.state, 'Incomnig message: ShippedReturn, State change: ' + pr.state.code + ' -> '  + newState.code, null);

                            reshareApplicationEventHandlerService.auditEntry(pr, pr.state, newState, 'Item(s) Returned by requester', null);
                            pr.state = newState;
                            pr.save(flush: true, failOnError: true);
                            break;
...
//                            reshareApplicationEventHandlerService.auditEntry(pr, newState, newState, 'Incomnig message: Cancel, State change: ' + pr.state.code + ' -> '  + newState.code, null);
     ...
     * IMPORTANT: If calling withLock true the caller must establish transaction boundaries
     */
    private PatronRequest lookupPatronRequest(String id, boolean withLock=false) {
    ...
    private PatronRequest lookupPatronRequestByPeerId(String id, boolean withLock) {
        PatronRequest result = PatronRequest.createCriteria().get {
